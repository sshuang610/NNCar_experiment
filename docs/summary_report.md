...
> 註：multi-seed 穩健性實驗仍在跑（4 seed 完成 2 個），本報告為「目前進度」快照；最終 best-of-N 聚合待補。

---

## 1. 一句話結論

**在競賽合約固定的條件下（6→6→4 網路、top-2 親代 + crossover/mutation），fitness 配方的影響遠小於 GA 隨機 seed 的運氣。**
唯一可靠的好成分是 **`safety` 獎勵**（讓車學會不撞牆）；但要真的跑出好模型，需要**多 seed 重試取最佳**，單一 seed 結果幾乎是運氣。
...
### 3.5 決定性發現：66% 是 seed 運氣，不是配方好
multi-seed 測試（同配方換 GA seed）：

| 配方 | seed 1234 | seed 5678 |
| --- | --- | --- |
| progress_safe | **0.662** | 0.110 |
| ps_hi_safe | 0.074 | **0.251** |
| 其餘 | ~0.05–0.09 | ~0.04–0.10 |

→ **progress_safe 的 66% 是在 seed 1234 中運氣好**；換 seed 就掉到 11%。反而 ps_hi_safe 在另一個 seed 跳到 25%。
**沒有任何配方跨 seed 穩定好**——結果由 GA 隨機 seed 主導，不是 fitness 配方。
...
- 66% 進度、0 碰撞、給足時間會完賽。
- **但請理解這是「好配方 + 好 seed」的組合**，換 seed 不保證重現 66%。
